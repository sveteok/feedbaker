...
export const main = async () => {
  ...
  try {
    if (process.env.SEED_TRUNCATE === "true") {
      console.log("Truncating existing data before seed...");
      await client.query("TRUNCATE feedback, sites, users RESTART IDENTITY CASCADE;");
    }
    ...
    if (validUsersData.length === 0) {
      throw new Error("No users inserted — seeding aborted.");
    }
    ...
    if (validSitesData.length === 0) {
      throw new Error("No sites inserted — skipping feedback seed.");
    }
